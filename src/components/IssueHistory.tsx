...
export const IssueHistory: React.FC<IssueHistoryProps> = ({ data, currentUser, onDelete }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Filters */}
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 p-2 border dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-red-500 outline-none bg-white dark:bg-slate-900 dark:text-slate-200"
          />
        </div>
        <select
          value={filterType}
          onChange={(e) => setFilterType(e.target.value)}
          className="p-2 border dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-red-500 outline-none bg-white dark:bg-slate-900 dark:text-slate-200"
        >
        ...
        <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-full p-2 border dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-red-500 outline-none text-sm bg-white dark:bg-slate-900 dark:text-slate-200"
            />
        </div>
      ...
      <div className="flex justify-end">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-lg text-sm text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
          >
      ...
      <div className="space-y-4">
        {filteredIssues.length > 0 ? (
          filteredIssues.map((issue) => (
            <div key={issue.id} className="bg-white dark:bg-slate-900 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow relative group">
              {isGestor && onDelete && (
                <button 
              ...
              <div className="mt-2">
                ...
                <p className="text-gray-700 dark:text-slate-300 text-sm leading-relaxed bg-gray-50 dark:bg-slate-900 p-3 rounded-lg border border-gray-100 dark:border-slate-700">
                  {issue.description}
                </p>
              </div>
            </div>
